...
impl OpenTypeFontBlob {
    pub(crate) fn new(data: FontData, index: u32) -> Option<Self> {
        ...
        // Some legacy Mac Japanese fonts use post format 4.0. skrifa can parse
        // these fonts and their Unicode cmap correctly, so keep them available
        // instead of falling back to a font with no matching Japanese glyphs.
